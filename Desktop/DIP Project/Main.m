...
% Select Carrier File
[filename, pathname] = uigetfile({'*.*'}, 'Select Carrier File');

if isequal(filename,0) || isequal(pathname,0)
    disp('User exited the operation.');
    return; 
end
...
% Select Secret File
[filename, pathname] = uigetfile({'*.*'}, 'Select Secret File');
...
img_smoothed = imgaussfilt(img_sharpened, 2);  
...
mse = sum(sum(sum((a - finalsteg).^2))) / (r * c * g);
